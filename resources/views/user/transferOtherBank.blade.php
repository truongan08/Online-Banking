            ...
            $(document).on('change','#bank',function () {
                $.ajax({
                   ...
                   success:function (res) {
                       ...
                       $('#message').html('<div role="alert" class="alert alert-danger margin-bottom-30" ><strong> Giới hạn chuyển tiền  '+min_amount+' {{$gnl->cur}} - '+max_amount+'  {{$gnl->cur}} .   Phí chuyển khoản là '+fixed_charge+' {{$gnl->cur}} phí cố định và  '+percent_charge+'% phí phần trăm.</strong></div>')
                   }
                });
            })
        });
